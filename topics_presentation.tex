...
\begin{frame}
\frametitle{What is complexity theory?}
Complexity theory is the study of what problems are easy to solve on a {\color{red} computer}.
...
\begin{itemize}
    ...
    \item How do we measure the difficulty of a problem?
\end{itemize}
\end{frame}
...
%------------------------------------------------
\section{How do we measure the difficulty of a problem?}
%------------------------------------------------
...
\begin{itemize}
    \item What is a computer? {\em Deterministic Turing Machine, Nondeterministic Turing Machine}
    \item What is a problem? {\em Deciding if a word is in a language, verifying that a word is in a language}
    \item How do we measure the difficulty of a problem? {\em Upper bound of time for an input of length $n$}
\end{itemize}
\end{frame}
...
\begin{algorithm}[H]
$V(w, c):$\\
\eIf{$M(w)$ accepts}{{\bf accept}}{{\bf reject}}
\end{algorithm}
...
We say that $f$ is {\em satisfiable} if we can assign $\bot$ or $\top$ to each variable such that $f = \top$.
...
\begin{itemize}
    ...
    \item Accept if the formula evaluates to $\top$, otherwise reject.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Cook-Levin Theorem}
Cook showed that any $NTM\, M$ that halts in polynomial time running on input $w$ can be converted to a polynomial length boolean formula such that:

\begin{itemize}
    \item If $M(w)$ accepts, then the formula is satisfiable
    \item and if $M(w)$ rejects, then the formula cannot be satisfied.
\end{itemize}
...
\begin{itemize}
    ...
    \item<3-> 0-1 Integer Programming
    \item<4-> Knapsack
    \item<5-> Tetris
    \item<6-> Lemmings
...
\begin{frame}
\frametitle{How do we measure the difficulty of a problem?}
